...
pub fn buildVersionInfo(allocator: Allocator, version: WindowsVersion, description: []const u8) ![]u8 {
    ...
    // Add FileDescription string
    const string_entries = [_]struct { key: []const u8, value: []const u8 }{
...
fn parseIconFileImpl(allocator: Allocator, data: []const u8) !struct {
    ...
    for (entries, 0..) |entry, i| {
        ...
        // Extract icon data
        const icon_data = try allocator.alloc(u8, entry.bytes_in_res);
        @memcpy(icon_data, data[entry.image_offset..][0..entry.bytes_in_res]);
...
pub fn parseResourceSection(allocator: Allocator, data: []const u8, virtual_base: u32) !ParsedResources {
    ...
    if (data.len < @sizeOf(ResourceDirectoryTable)) return result;
                    ...
                    switch (type_id) {
                        @intFromEnum(RT.ICON) => {
                            if (!name_entry.entry.name_offset.to_string) {
                                const icon_data = try allocator.dupe(u8, resource_data);
                                try icon_list.append(.{ .id = @intCast(name_entry.entry.integer_id), .data = icon_data });
                            }
                        },
                        @intFromEnum(RT.GROUP_ICON) => {
                            if (!name_entry.entry.name_offset.to_string) {
                                const group_data = try allocator.dupe(u8, resource_data);
                                try group_icon_list.append(.{ .id = @intCast(name_entry.entry.integer_id), .data = group_data });
                            }
                        },
                        @intFromEnum(RT.VERSION) => {
                            result.version_info = try allocator.dupe(u8, resource_data);
                        },
                        else => {},
                    }
                }
...
pub const TestingAPIs = struct {
    ...
    pub fn parseResources(globalThis: *JSGlobalObject, callframe: *CallFrame) bun.JSError!JSValue {
        ...
        // Find .rsrc section
        var rsrc_section_index: ?usize = null;
        for (pe_file.section_headers.items, 0..) |section, i| {
        ...
        const section_index = rsrc_section_index orelse {
            return globalThis.throwError(error.ResourceSectionNotFound, "No .rsrc section found");
        };

        const rsrc_section = pe_file.section_headers.items[section_index];
        const rsrc_data = pe_file.sections_data.items[section_index];

        // Parse resources
        const resources = parseResourceSection(allocator, rsrc_data, rsrc_section.virtual_address) catch |err| {
            return globalThis.throwError(err, "Failed to parse resources");
        };
        defer {
            for (resources.icons) |icon| allocator.free(icon.data);
            allocator.free(resources.icons);
            for (resources.group_icons) |group| allocator.free(group.data);
            allocator.free(resources.group_icons);
            if (resources.version_info) |v| allocator.free(v);
        }
