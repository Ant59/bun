...
pub const PEFile = struct {
    ...
    pub fn addBunSection(
        ...
        for (pe.section_headers.items) |section| {
            const virtual_end = section.virtual_address +% alignSize(if (section.virtual_size > 0) section.virtual_size else section.size_of_raw_data, pe.optional_header.section_alignment);
        ...
        @memcpy(output_data[section_headers_offset..][0..existing_sections_size], file_data[section_headers_offset..][0..existing_sections_size]);
        ...
        if (remaining_size > 0 and remaining_start < file_data.len) {
            ...
            @memcpy(output_data[remaining_start..][0..copy_size], file_data[section_headers_offset +% existing_sections_size ..][0..copy_size]);
        }
    ...
    pub fn updateResourceSection(
        ...
        if (rsrc_section) |section| {
            ...
            // Copy existing section headers
            const existing_sections_size = @as(u32, pe.pe_header.number_of_sections) *% @sizeOf(SectionHeader);
            @memcpy(modified_data[section_headers_offset..][0..existing_sections_size], file_data[section_headers_offset..][0..existing_sections_size]);
            ...
            if (remaining_start < orig_end) {
                ...
                @memcpy(modified_data[remaining_start..][0..remaining_size], file_data[@min(section_headers_offset +% existing_sections_size, file_data.len)..][0..remaining_size]);
            }
